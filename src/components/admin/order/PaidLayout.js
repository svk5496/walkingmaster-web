...
import OrderPaidList from "./PaidList";
...
const SEE_PAID_ORDER_QUERY = gql`
  query seeOrders(
    $firstName: String
    $phone: String
    $startDate: String
...
function OrderPaidLayout() {
  ...
  const { data } = useQuery(SEE_PAID_ORDER_QUERY, {
    variables: {
    ...
    <>
      <Layer>
        ...
        <Container>
          ...
          {/* 컨텐트 시작 */}
          <OrderPaidList data={data}></OrderPaidList>
        </Container>
      </Layer>
    </>
  );
...
export default OrderPaidLayout;
